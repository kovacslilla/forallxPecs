...
Az evkivalens szabályok hasonlóak lesznek, mint az implikáció kétszeres változatai.
...
Például a $W \eiff X$ származtatáshoz be kell bizonyítani $X$-et $W$ feltételezésével és $W$-t $X$ feltételezésével. Az ekvivalens bevezetési szabály ({\eiff}I) két részbizonyítást igényel. A részbizonyítások bármilyen sorrendben elvégezhetők és a második részbizonyításnak nem kell rögtön az első után következnie-, de vázlatosan a szabály így működik:
...
Az ekvivalens eliminációs szabály ({\eiff}E) kicsit többet tesz lehetővé, mint az implikációs szabály. Ha az ekvivalencia bal oldali része megvan, abból levezethető a jobb oldal, ha a jobb oldali rész van meg, abból a bal oldal vezethető le. Ez a szabály:
...
\item[] Tegyük fel, hogy van egy legnagyobb természetes szám. Híjuk $A$-nak.
...
\item[] Egyértelmű, hogy $A+1 > A$.
\item[] So there is a natural number greater than $A$.
\item[] Tehát van egy természetes szám, ami nagyobb $A$-nál.
\item[] This is impossible, since $A$ is assumed to be the greatest natural number.
\item[] Ez lehetetlen, mivel azt feltételeztük, hogy $A$ a legnagyobb természetes szám.
\item[\therefore] There is no greatest natural number.
\item[\therefore] Nincs legnagyobb természetes szám.
\end{earg}
...
Ezt az érvelési formát hagyományosan \emph{reductio}-nak nevezzük. Teljes latin neve \emph{reductio ad absurdum}, ami azt jelenti 'visszavezetés az abszurdra'. Egy reductioban az érvek kedvéért feltételezzünk valamit - például, hogy van a legnagyobb természetes szám. Aztán megmutatjuk, hogy a feltételezés két ellentmondásos mondathoz vezet - például, hogy $A$ a legnagyobb természetes szám, és hogy nem. Ily módon megmutatjuk, hogy az eredeti feltételezés hamis volt.
